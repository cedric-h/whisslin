...
pub fn collision(world: &mut World) {
    ...
    world
        ...
        .query::<&Cuboid<f32>>()
        .into_iter()
        .filter(|(id, _)| world.ecs.get::<CollisionStatic>(*id).is_err())
        // get the ents that need to be moved
        .filter_map(|(id, cuboid)| {
            let iso = world.ecs.get::<Iso2>(id).ok()?;
            for (o_id, (o_iso, o_cuboid)) in world.ecs.query::<(&Iso2, &Cuboid<f32>)>().iter() {
                if id != o_id {
        ...
        .for_each(|(collided_id, normal)| {
            let mut iso = world.ecs.get_mut::<Iso2>(collided_id).unwrap_or_else(|e| {
                panic!(
